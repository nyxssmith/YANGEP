...
GreenFlashEffect::GreenFlashEffect(CF_Shader shader)
	: m_active(false)
	, m_elapsed(0.0f)
	, m_flashes(3)
...
void GreenFlashEffect::trigger(int flashes, float totalDuration, float maxIntensity)
{
	m_flashes = flashes;
...
void GreenFlashEffect::update(float dt)
{
	if (!m_active) return;
	m_elapsed += dt;
...
bool GreenFlashEffect::isActive() const
{
	return m_active;
}

float GreenFlashEffect::computeIntensity() const
{
...
void GreenFlashEffect::beginDraw()
{
	m_shaderPushed = false;
	if (!m_active) return;
