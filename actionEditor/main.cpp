// todo window with action list
//  can make a new action
//  can click on the grid to toggle tile added to action
//  can save action to folder
...
int main(int argc, char *argv[])
{
    ...
    RealConfigFile preConfig("assets/action-editor-window-config.json");
    ...
    CF_Result result = make_app("Action Editor", 0, 0, 0, windowWidth, windowHeight, options, argv[0]);
    cf_app_init_imgui();
    if (is_error(result))
        return -1;
    ...
    DataFile windowConfig("/assets/action-editor-window-config.json");
    ...
    LevelV1 level("/assets/Levels/actionEditor");
    ...
    float startTileX = 17.0f;
    float startTileY = 17.0f;
